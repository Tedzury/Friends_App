...
function handleCardClick({target}) {
    ...
    target : target.parentElement.classList.contains('person__card')?
    target.parentElement : target.parentElement.parentElement.classList.contains('person__card')?
    target.parentElement.parentElementclassList : undefined;

    neededElement === undefined ? null : neededElement.classList.toggle('person__card_spreaded');    
}
...
function handleNameSearch({target}) {
    ...
    personsToRender.forEach( user => {
        const isVisible = user.firstName.toLowerCase().includes(value) || user.lastName.toLowerCase().includes(value);
...
function applyFilters() {
    ...
    let genderFilter; 
    filterRadioBtns.forEach( radio => {if (radio.checked) genderFilter = radio.value});
    let ageOrNameSort;
    sortingRadioBtns.forEach( radio => {if (radio.checked) ageOrNameSort = radio.value});
    ...
    function sortingByAgeOrName() {
        switch(ageOrNameSort) {
            ...
            case 'randomSort':
                return sortedArray.sort(function () {return 0.5 - Math.random()});
            case 'ascendingSort':
                return sortedArray.sort((a, b) => a.firstName.localeCompare(b.firstName));
            case 'descendingSort':
                return sortedArray.sort((a, b) => a.firstName.localeCompare(b.firstName)).reverse();
            default:
                break;
        }
    } 
