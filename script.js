...
sortingForm.addEventListener('change', handleSortAndFiler);
...
function handleSortAndFiler() {

  let genderFilter = [...filterRadioBtns].find((radio) => { if (radio.checked) return radio }).value;
  let ageOrNameSort = [...sortingRadioBtns].find((radio) => { if (radio.checked) return radio }).value;
